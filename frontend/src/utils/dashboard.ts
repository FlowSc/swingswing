...
export function enrichPlanPercentRow(row: Record<string, unknown>) {
  ...
  return {
    ...
    진입가: row.entry,
    손절가: row.stop_loss,
    손절률: formatPlanPct(exitPlan.stop_loss_pct) || formatPercentFromEntry(stopLoss, entry),
    "2차익절가": row.take_profit_2,
    "2차익절률": formatPlanPct(exitPlan.take_profit_2_pct) || formatPercentFromEntry(takeProfit2, entry),
    종목코드: row.code,
